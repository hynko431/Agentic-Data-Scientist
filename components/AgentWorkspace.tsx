...
import { Play, Settings, Activity, Download, FileCode, BookOpen, ChevronLeft, LayoutDashboard } from 'lucide-react';
import Terminal from './Terminal';
import AgentStatusPanel from './AgentStatus';
import FilePanel from './FilePanel';
import { LogMessage, AgentRole, AgentStatus, AnalysisPlanStep, FileData, Artifact, DashboardMetrics } from '../types';
...
const AgentWorkspace: React.FC<AgentWorkspaceProps> = ({ onBack, onUpdateDashboard }) => {
  ...
  const runOrchestration = async () => {
    ...
    // 3. Summary & Dashboard Generation Phase
    ...
    addLog(AgentRole.SYSTEM, "Generating Dashboard Analysis...", 'info');
    const dashboardData = await generateDashboardData(executionContext);
    onUpdateDashboard(dashboardData);
    addLog(AgentRole.SYSTEM, "Dashboard updated with new analysis metrics.", 'success');
  ...
  return (
    <div className="flex flex-col h-screen bg-background text-text overflow-hidden w-full">
      {/* Header */}
      <header className="h-14 border-b border-surfaceHighlight bg-surface/50 backdrop-blur flex items-center justify-between px-6 shrink-0 z-10">
        ...
        <div className="flex items-center gap-4">
          <button 
            onClick={onBack}
            className="flex items-center gap-2 px-3 py-1.5 text-xs font-medium text-white bg-secondary hover:bg-secondary/80 rounded-md transition-all shadow-lg"
          >
                ...
                <div className="flex-1 overflow-auto p-4 custom-scrollbar">
                   {artifacts.length > 0 ? (
                     <div className="space-y-2">
                       {artifacts.map(art => (
                         <div key={art.id} className="bg-surface/50 border border-surfaceHighlight rounded p-3 hover:border-primary/30 transition-colors cursor-pointer group relative">
                           <div className="flex items-center justify-between mb-1">
                             ...
                             <button 
                                ...
                                className="opacity-0 group-hover:opacity-100 p-1 hover:bg-surfaceHighlight rounded text-muted hover:text-white transition-all"
                                title="Download File"
                             >
                                <Download className="w-3.5 h-3.5" />
                             </button>
